...
The dataset includes both active listings and recently sold properties. Price figures reflect both listing prices and final sale prices as provided in the source data. The analysis covers all five required questions, including median vs average price, price by location, price drivers, correlations between characteristics, and a detailed Florida market analysis. Additional statistical analysis, including single-variable and multiple linear regression, IQR-based outlier detection, and predictive modeling, was performed to provide deeper analytical rigor.
...
A separate IQR-filtered table (`dbo.realtor_iqr`, 1,297,370 records — 87.5% of the clean data) was created for all regression analysis. The fully cleaned dataset of 1,481,944 records was retained for all descriptive analysis to accurately represent the complete market, including luxury segments. This separation is the statistically correct approach: outliers belong in descriptive statistics but bias linear regression models.
...
The $201,549 gap indicates a right-skewed distribution driven by luxury outliers. The median is the more accurate representation of what a typical buyer pays. Average price is used as the primary metric throughout the descriptive analysis because it is the standard metric for comparative market analysis across states and cities.
...
|---|---|---|---|---|---|---|
| House Size | $138.95/sqft | $137,683 | 0.4293 | 0.1843 | 541.35 | p < 0.001 |
| Bedrooms | $61,738/bed | $193,895 | 0.2421 | 0.0586 | 284.21 | p < 0.001 |
| Bathrooms | $119,034/bath | $114,675 | 0.4295 | 0.1845 | 541.75 | p < 0.001 |

Running regression on the IQR-filtered table produces realistic, defensible coefficients. House size (~$139/sqft) and bathrooms (~$119K) emerge as the strongest single predictors, each explaining roughly 18% of price variance. Bedrooms alone explain about 6%.

**Multiple Regression**

Single-variable models suffer from omitted variable bias: because larger homes tend to have more bedrooms, bathrooms, and square footage simultaneously, each isolated model overstates its predictor's effect. A multiple regression solves all three coefficients at once so the variables share the price, revealing each feature's true marginal value.
...
**Model R² = 0.2229** · Adjusted R² = 0.2229 · F = 124,066 · p < 0.001

```
Price = $120,933 + ($100.95 × sqft) − ($27,133 × beds) + ($74,086 × baths)
```

The combined model explains 22.3% of price variance — more than any single predictor alone, confirming the variables work better together. Square footage emerges as the true primary driver (~$101/sqft), and each additional bathroom adds roughly $74K. The bedroom coefficient turns **negative**: once square footage is held constant, adding bedrooms means smaller rooms, which slightly reduces value. This is a well-documented result in real estate economics and demonstrates the model correctly isolating marginal effects.
...
Prices in this table are based on observed market averages for each home profile rather than regression extrapolation, making them reliable for practical reference.
...
Bedrooms, bathrooms, and square footage are strongly correlated (multicollinearity) — this is precisely why the multiple regression in 3.3 is needed to isolate true marginal effects. Acre lot size shows a surprising inverse relationship at the low end: 1-bed homes average 37.59 acres vs 10.43 for 3-bed homes, explained by rural agricultural parcels with modest dwellings.
...
Florida commands a 12.6% price premium despite homes averaging 232 sqft smaller, meaning buyers pay more per square foot — a premium driven by climate, coastal access, and in-migration. Internal variation is extreme: Jacksonville ($334,260) to Miami Beach ($2,170,059).
...
| 6 | Regression Analysis | IQR-filtered single + multiple regression with R² | Q2c extended |
...
This analysis of U.S. real estate listings produced several significant findings about the American housing market.
...
A multiple regression model on the IQR-filtered data (R² = 0.2229) outperforms every single-variable model and corrects the omitted variable bias inherent in isolated regressions. Square footage is the true primary price driver (~$101/sqft), each bathroom adds ~$74K, and bedrooms show a negative marginal effect once size is held constant — adding bedrooms without space yields smaller rooms. All predictors are significant at p < 0.001.

Florida commands a 12.6% price premium despite offering homes 232 sqft smaller than the national average, with internal variation ranging from Jacksonville ($334,260) to Miami Beach ($2,170,059).

The combination of SQL Server for data management and Power BI for visualization, together with IQR-based outlier detection and both single-variable and multiple regression computed from scratch in T-SQL, provides a comprehensive analytical framework that goes well beyond basic descriptive statistics.
...
| `06_analysis_queries.sql` | Q2a–Q2e analysis queries |
| `07_regression.sql` | IQR table + single-variable regression |
| `08_multiple_regression.sql` | Multiple regression (all three variables) |
